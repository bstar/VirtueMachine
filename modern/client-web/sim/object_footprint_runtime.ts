...
export type ObjectFootprintTileRuntime = {
  x: number;
  y: number;
  tileId: number;
};
...
export function objectFootprintTilesRuntime(
  ...
  const sx = wrap10(ox);
  const sy = wrap10(oy);
  const baseTileId = tileId & 0xffff;
  const tf = tileFlagsForTile(baseTileId) | 0;
  const out: ObjectFootprintTileRuntime[] = [{ x: sx, y: sy, tileId: baseTileId }];
  if (tf & 0x80) {
...
export function isImplicitSolidObjectTileRuntime(
  ...
  const type = objType & 0x03ff;
  if (OBJECT_TYPES_DOOR.has(type)) {
    return false;
  }
  const tf = tileFlagsForTile(tileId & 0x07ff) | 0;
  if ((tf & 0x20) !== 0) {
    return true;
  }
  if ((tf & 0xc0) === 0) {
