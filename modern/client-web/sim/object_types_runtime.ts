...
const OBJECT_TYPES_SIGN = u6ObjectTypeSet(OBJECT_TYPE_SIGN_VALUES);
const OBJECT_TYPES_ENV_FIXTURE = u6ObjectTypeSet(OBJECT_TYPE_ENV_FIXTURE_VALUES);
...
export function isLikelyPickupObjectTypeRuntime(type: number, typeWeights?: ArrayLike<number> | null): boolean {
  ...
  if (OBJECT_TYPES_SIGN.has(t)) return false;
  if (OBJECT_TYPES_ENV_FIXTURE.has(t)) return false;
  if (OBJECT_TYPES_STRUCTURAL_NON_PICKUP.has(t)) return false;
  if (typeWeights && (Number(typeWeights[t]) | 0) === 0 && !OBJECT_TYPES_ZERO_WEIGHT_TAKEABLE.has(t)) return false;
  return true;
}
