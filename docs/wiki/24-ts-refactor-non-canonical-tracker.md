...
- Note: many runtime signatures still use broad object typing to keep slices moving. World object take keys and inventory projection now use typed net-runtime boundary sources, with unknown server inventory JSON decoded before projection; net presence remote-player and authoritative-NPC rows now use explicit JSON decoders before projection/application, character-list rows decode before selection, account user payload reads use typed accessors, message-log raw entries decode before windowing/snapshot normalization, snapshot payload saved-tick/base64 reads go through typed accessors, the shared net request layer now models JSON scalars/arrays explicitly, session state now uses the typed remote-player shape, and login state application shares the auth-runtime login payload contract.
