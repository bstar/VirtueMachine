...
- Status: `temporary`
- Location examples: `inventory*`, `range*`, `target*` wrappers
- Note: queue command mutation now lives in `sim/queue_runtime.ts`, and door toggle lookup/mutation now lives in `sim/door_runtime.ts`; `app.ts` still keeps some thin wrapper functions delegating to other typed runtime modules.
